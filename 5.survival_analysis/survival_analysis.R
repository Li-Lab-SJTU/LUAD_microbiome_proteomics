...
for (name in c("Cytomegalovirus_g",
  ...
  surv_plot <- ggsurvplot(fit, data = res.cat,
                          pval = TRUE, conf.int = FALSE,
                          legend.title = name,
                          xlab = "OS (month)", ylab = "Survival Probability",
                          ggtheme = theme_bw(),
                          font.main = 14, font.submain = 14, font.caption = 14,
                          font.x = 14, font.y = 14,
                          legend.labs = c("High",  "Low"),
                          legend.fontsize = 14,
                          legend.position = "right",
                          legend.box = "vertical",
                          legend.title.theme = element_text(face = "bold"),
                          legend.text = element_text(face = "italic"),
                          surv.median.line = "hv",
                          palette = c("#0066CC", "#CC0000"),
                          tables.theme = theme_cleantable(),
                          tables.col = c("strata", "n", "p", "median"),
                          break.x.by = 10,
  )
...
for (name in c("Firmicutes_p", "Gammaproteobacteria_c")) {
  ...
  surv_plot <- ggsurvplot(fit, data = res.cat,
                          pval = TRUE, conf.int = FALSE,
                          legend.title = name,
                          xlab = "DFS (month)", ylab = "Survival Probability",
                          ggtheme = theme_bw(),
                          font.main = 14, font.submain = 14, font.caption = 14,
                          font.x = 14, font.y = 14,
                          legend.labs = c("High",  "Low"),
